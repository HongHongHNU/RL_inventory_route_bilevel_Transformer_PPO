...
class Decoder1(nn.Module):
    ...
    def forward(self, encoder_inputs, pool, actions_old, n_steps, batch_size,n_nodes, greedy=False, _action=False):
...
class Model(nn.Module):
    ...
    def forward(self, datas, actions_old, n_steps, batch_size, greedy, _action):
        ...
        pooled = x.mean(dim=1)
        actions, log_p, entropy, dists = self.decoder(x, pooled, actions_old, n_steps, batch_size, n_steps,greedy, _action)
...
class Agentppo:
    ...
    def update(self, memory, epoch,n_nodes):
        ...
        for i in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                loss = torch.min(surr1, surr2) + 1 * mse_loss - self.entropy_value * entropy
